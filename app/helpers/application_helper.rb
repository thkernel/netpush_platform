module ApplicationHelper
  ...
  def user_avatar(user, alt_tag, class_name)
	...
	def not_need_workplace?(speciality_id)
		...
		def count_structure_type(id)
			structures = Structure.where(structure_type_id: id)
			structures.count
		end

		def all_specialities
			Speciality.last(10).reverse
		end

		def count_speciality(id)
			
			specialities = Profile.where(speciality: Speciality.find(id).name)
			specialities.count
		end
